...
Therefore, grant $\textit{p}$ by $C$ to $D$ is also revoked. As $D$ is still authorized exercise and delegation of $\textit{p}$ directly by $A$, no further changes in grant diagram would be necessary.
Hence, $C$ would still be able to exercise privilege $\textit{p}$. This also holds true for $E$.
