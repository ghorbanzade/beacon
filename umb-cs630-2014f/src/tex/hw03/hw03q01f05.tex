...
\begin{tikzpicture}[scale=0.6, transform shape, node distance=3cm, every edge/.style={link}]
...
\node[attribute] (addressid) [below of = address] {\key{ID}} edge (address);

\node[relationship] (has) [below right of = address] {Has} edge node[auto,swap] {1} (address);
